...
- **坡度是選填欄位 `incline_pct`(跑步機坡度,單位 %)**:跟 `avg_hr_bpm` 一樣的規則——
  照片/敘述有給就記,沒有就整個欄位省略(不要填 `null`)。坡度 0 是有意義的實測值,可以填 `0`。
  目前 dashboard 還沒呈現它,但資料先存著。
- **卡路里是選填欄位 `calories_kcal`(單位 kcal)**:規則同上——有就記,沒有就整個欄位省略。
  注意 Vision 跑步機第 4 格上排是火焰(卡路里)、下排標 `METS`,**只有火焰那個指示燈亮時才是卡路里**;
  METS 是強度指數(個位數等級),看到三位數就是卡路里。
