...
- **HIIT 沒有專屬欄位**：間歇結構（快幾分鐘／慢幾分鐘／幾循環、各段速度）寫進 `note` 的自由文字，
  不拆成結構化欄位——拆了會跟 `duration_min` 變成兩個互相打架的來源，而且目前沒有頁面元素會讀它。
  HIIT 最該問到的是 `max_hr_bpm`，那是它唯一有意義的強度指標（Captain 頁的 HIIT PR 榜就吃這一欄）。
