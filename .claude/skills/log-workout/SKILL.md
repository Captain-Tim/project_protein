...
- **HIIT 多五個必填欄位**：`work_speed_kmh`（衝刺段速度）、`work_min`（衝刺段長度，分鐘）、
  ...
  **前四欄如果截圖/使用者輸入沒給，預設沿用該人最近一筆 HIIT 紀錄的數值**，不要整組空白問使用者。
  直接查：

  ```bash
  grep -l '"exercise": "HIIT"' data/<人名>/*.json | tail -1
  ```
  ...
  坡度不套用預設、不預設 0：坡度是每次實際看到的檔位，一般 HIIT 不開坡度，但**沒開就是實測 0、
  要記 `0`**，沒看到數字才是省略欄位。
...
- 以下三個是**選填欄位**，共通規則：照片/敘述有給就記，沒有就**整個欄位省略**（不要填 `null`）。
  ...
  **`avg_hr_bpm`、`max_hr_bpm`、`calories_kcal` 這三個如果同時有 Apple Watch 截圖（Fitness/健身
  App 的訓練摘要），一律採用 Watch 的數字，不要看跑步機螢幕。** Watch 貼身量測，跑步機用手握感應器
  量到的心率本來就不準，卡路里也是機器用固定公式估的。兩邊都有給、數字對不上時，用 Watch 的，
  不用平均或並記兩個數字。`incline_level`、`resistance_level` 這兩個 Watch 量不到，仍然看跑步機/
  健身車螢幕。
  **`calories_kcal` 只記 Watch 畫面上的 total calories（總卡路里），不要記 active calories 或
  其他拆分數字**——那些欄位不存，跟卡路里放進同一個數字會混淆量測口徑。
