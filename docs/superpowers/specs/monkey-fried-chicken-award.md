...
- 一張券一列：🍗 票根 + 撕線虛線 + `EARNED <取得日>`
  ...
  - 已使用：整列灰階，右邊一枚紅框 `USED <日期>` 戳記。**列上不放取得日與 `note`**，
    改成點券跳出詳情小卡（見下）
...
手機 393px 一列只有 361px。已使用的券要放五樣東西：🍗 票根、取得日、`SPECIAL` 徽章、
`USED` 戳記、`note`。徽章與戳記都 `white-space: nowrap`，縮不了，全部排一列會溢出——
頁面真的可以左右捲，不是只有文字被裁掉。

所以取得日與 `note` 收進小卡：整頁模糊底 + 置中卡，內容是取得日、使用日、`note`
（沒有 `note` 就不畫那一行）。開關行為與 `dashboard-monkey.html` 的 `#dayModal` 一致——
點背景或 `×` 關閉，**沒有 Esc**。
