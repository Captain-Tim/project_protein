...
- 所有桌機的左右並排一律改上下堆疊。完整的區塊順序見 `docs/page-layout.md`，
  原則是每天要看的在上、回顧型的在下
...
- **選配的量測欄位**：`avg_hr_bpm`（平均心率 bpm）、`incline_level`（跑步機坡度**檔位**，不是百分比）、
  `resistance_level`（健身車阻力**檔位**）、`calories_kcal`。四個都是量到的來源值、不是推導值，所以可以存。
  有量到才填，沒有就整個欄位省略（不要填 `null`）。`build_dashboard.js` 不驗這幾欄。
  `incline_level` 與 `resistance_level` 的 `0` 都是有效實測值，判斷一律用 `!= null`。
  兩者**不共用一欄**：跑步機的坡度與健身車的阻力刻度不同、器材也不同，混在一起事後分不出來。
